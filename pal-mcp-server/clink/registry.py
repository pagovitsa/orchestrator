...
ALLOWED_CLIENTS_ENV_VAR = "CLINK_ALLOWED_CLIENTS"
DISABLED_CLIENTS_ENV_VAR = "CLINK_DISABLED_CLIENTS"
...
class ClinkRegistry:
    ...
    def _apply_client_filters(self) -> None:
        allowed = self._parse_client_filter(get_env(ALLOWED_CLIENTS_ENV_VAR))
        disabled = self._parse_client_filter(get_env(DISABLED_CLIENTS_ENV_VAR))

        if allowed:
            self._clients = {key: value for key, value in self._clients.items() if key in allowed}
        if disabled:
            self._clients = {key: value for key, value in self._clients.items() if key not in disabled}

        if not self._clients:
            raise RegistryLoadError(
                "No CLI clients remain after applying clink filters. Check "
                f"{ALLOWED_CLIENTS_ENV_VAR}/{DISABLED_CLIENTS_ENV_VAR}."
            )

    def _parse_client_filter(self, value: str | None) -> set[str]:
        if not value:
            return set()
